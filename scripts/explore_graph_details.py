...
def explore_graph_details():
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            defect_probs = outputs['patch_defect_probs'][0].cpu().numpy()  # (1296, 4)
            ...
            class_names = ['Perfect', 'Vacancy', 'Interstitial', 'Grain Boundary']  # Guessing
            ...
            # Property predictions breakdown
            prop_preds = outputs['property_predictions'][0].cpu().numpy()
            prop_uncert = outputs['property_uncertainty'][0].cpu().numpy()
            
            prop_names = ['Young_Modulus', 'Poisson_Ratio', 'Thermal_Conductivity', 'Electrical_Conductivity', 'Bandgap']
            print(f"\n⚙️  MATERIAL PROPERTY PREDICTIONS:")
            for i, (name, pred, uncert) in enumerate(zip(prop_names, prop_preds, prop_uncert)):
                print(f"   {name}: {pred:.4f} ± {uncert:.4f}")
            ...
            hotspots = np.where(attention > threshold)
            print(f"   High attention regions: {len(hotspots[0])} pixels above threshold ({threshold:.4f})")
